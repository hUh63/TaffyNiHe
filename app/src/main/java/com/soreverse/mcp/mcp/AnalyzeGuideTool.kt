 ...
 *    （so_open → 分析 → 交叉引用 → 搜索 → 模拟执行 → report）收集证据，然后由外部 AI 自行综合报告。
    ...
    val guide: ToolHandler = object : ToolHandler {
        ...
        private fun buildGuide(
            ...
            try {
                ...
                statsHint = " ELF结构/节区/符号可经 analyze_elf 获取"
            } catch (_: Throwable) { }
            ...
            return if (zh) {
                buildString {
                    ...
                    append("【第 0 步 · 确认目标】\n")
                    append("  目标已打开，workspaceId=$wsPrefix。后续每个非 so_open 工具都要带上 workspaceId=$wsPrefix。\n\n")
                    append("【第 1 步 · 静态结构】\n")
                    append(step(1, "analyze_elf", "workspaceId=$wsPrefix view=full", "获取 ELF 节区、程序头、符号、重定位、动态段，了解文件的整体布局和导入导出")).append("\n")
                    append(step(2, "read_stats", "workspaceId=$wsPrefix", "确认统计概览：节区/符号/字符串数量，判断是否 strip")).append("\n\n")
                    append("【第 2 步 · 函数与控制流】\n")
                    append(step(3, "analyze_functions", "workspaceId=$wsPrefix", "列出 Rizin 发现的所有函数及地址/大小，定位关键函数")).append("\n")
                    append(step(4, "analyze_cfg", "workspaceId=$wsPrefix locator=<函数>", "对关键函数生成控制流图，理解执行逻辑与分支")).append("\n\n")
                    append("【第 3 步 · 交叉引用与调用关系】\n")
                    append(step(5, "analyze_xrefs", "workspaceId=$wsPrefix locator=<符号/函数> direction=to", "查关键函数的调用来源，追本溯源")).append("\n\n")
                    append("【第 4 步 · 字符串与可信特征】\n")
                    append(step(6, "search_strings", "workspaceId=$wsPrefix prefix=<关键词>", "搜 URL/域名/密钥/错误提示等，识别加密或网络行为")).append("\n")
                    append(step(7, "search_bytes", "workspaceId=$wsPrefix pattern=<十六进制>", "按机器码特征定位函数或用例")).append("\n\n")
                    append("【第 5 步 · 反汇编与密码学】\n")
                    append(step(8, "read_disasm", "workspaceId=$wsPrefix locator=<函数>", "反汇编关键函数，必要时开启 Ghidra 伪代码")).append("\n")
                    append(step(9, "analyze_crypto", "workspaceId=$wsPrefix", "扫描 AES/RSA/ECC 常量与高熵区，识别加密逻辑")).append("\n\n")
                    append("【第 6 步 · 动态验证（如需）】\n")
                    append(step(10, "emulate_call / unidbg_*", "workspaceId=$wsPrefix symbolName=<导出函数>", "模拟执行 JNI_OnLoad / Java_* / 关键导出，验证行为")).append("\n\n")
                    append("【第 7 步 · 综合报告】\n")
                    append(step(11, "analysis_report", "workspaceId=$wsPrefix", "生成引擎侧的综合报告（与本攻略互补）")).append(" 然后你基于以上所有证据，输出最终深度分析报告。\n")
                ...
                buildString {
                    ...
                    append("  Target opened, workspaceId=$wsPrefix. Pass workspaceId=$wsPrefix on every tool except so_open.\n\n")
                    append("[Step 1 · Static structure]\n")
                    append(step(1, "analyze_elf", "workspaceId=$wsPrefix view=full", "sections, program headers, symbols, relocations, dynamic entries; understand layout, imports/exports")).append("\n")
                    append(step(2, "read_stats", "workspaceId=$wsPrefix", "overview counts; detect stripped")).append("\n\n")
                    append("[Step 2 · Functions & control flow]\n")
                    append(step(3, "analyze_functions", "workspaceId=$wsPrefix", "list Rizin functions with addresses; locate key logic")).append("\n")
                    append(step(4, "analyze_cfg", "workspaceId=$wsPrefix locator=<fn>", "control-flow graph of key functions")).append("\n\n")
                    append("[Step 3 · Cross-references]\n")
                    append(step(5, "analyze_xrefs", "workspaceId=$wsPrefix locator=<sym> direction=to", "find callers of key functions; trace roots")).append("\n\n")
                    append("[Step 4 · Strings & search]\n")
                    append(step(6, "search_strings", "workspaceId=$wsPrefix prefix=<keyword>", "URLs, domains, keys, messages; detect network/crypto")).append("\n")
                    append(step(7, "search_bytes", "workspaceId=$wsPrefix pattern=<hex>", "locate code/uses by byte signature")).append("\n\n")
                    append("[Step 5 · Disasm & crypto]\n")
                    append(step(8, "read_disasm", "workspaceId=$wsPrefix locator=<fn>", "disassemble key functions; use Ghidra pseudocode if needed")).append("\n")
                    append(step(9, "analyze_crypto", "workspaceId=$wsPrefix", "scan AES/RSA/ECC constants & high-entropy regions")).append("\n\n")
                    append("[Step 6 · Dynamic (optional)]\n")
                    append(step(10, "emulate_call / unidbg_*", "workspaceId=$wsPrefix symbolName=<export>", "emulate JNI_OnLoad / Java_* / key exports to verify")).append("\n\n")
                    append("[Step 7 · Final report]\n")
                    append(step(11, "analysis_report", "workspaceId=$wsPrefix", "engine-side full report, complementary to this guide")).append(" Then produce your final deep-analysis report from all evidence above.\n")
